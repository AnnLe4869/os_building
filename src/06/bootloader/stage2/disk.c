...
/**
 * @brief read the data from sectors and write it to a buffer
 * 
 * @param disk 
 * @param lba 
 * @param sectors number of sectors to read
 * @param dataOut where to write the data to
 * @return true 
 * @return false 
 */
bool DISK_ReadSectors(DISK* disk, uint32_t lba, uint8_t sectors, void far* dataOut)
{
    ...
    // try for couple of times, as floppy disk can be unreliable
    for (int i = 0; i < 3; i++)
    {
